// WordGap pi extension: report turn lifecycle to the local WordGap daemon.
...
export default function wordgap(pi: any) {
  const PORT = 8765;
  const post = (event: string) => {
    try {
      ...
      fetch(`http://127.0.0.1:${PORT}/hook/pi/${event}?src=wordgap`, {
        method: "POST",
        body,
