...
### 9.1 M3 后演进路线(2026-08-27 定稿)

底层逻辑:**dsh 给渠道,Claude Code/Codex 给市场——先拿下有渠道的,再降低有市场那边的摩擦。**
所有面板决策过同一个过滤器:"agent 跑完时,用户愿不愿意立刻离开这个面板"。
...
**不做清单(边界,与做什么同等重要)**:云同步/账号系统;通用背单词 app(离开"agent 间隙"
场景即无差异化);macOS/Linux(直到有真实需求量);移动端;浏览器扩展/Web 版 agent 适配;
自建词库内容(永远用开源词库)。
