...
~/.claude/settings.json,则已装的 vibegap 钩子天然生效(agent 显示为 claude-code);
若 bridge 有独立配置路径,用:
...
```bash
dsh plugin --profile web add "github:<owner>/dsh-vibegap#main"
```
...
GitHub 建仓 `dsh-vibegap` + 打 `dsh-plugin` topic + 按 awesome-deepseek-harness
